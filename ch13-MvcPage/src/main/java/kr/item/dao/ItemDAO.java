...
public class ItemDAO {
	...
	public void insertItem(ItemVO item)
		...
		try {
			...
			sql = "INSERT INTO zitem (item_num,"
				+ "name,price,quantity,photo1,"
				+ "photo2,detail,status) VALUES ("
				+ "zitem_seq.nextval,?,?,?,?,?,?,?)";
	...
	public int getItemCount(String keyfield,
			                 String keyword,
			                 int status)
	                       throws Exception{
		Connection conn = null;
	...
	//관리자/사용자 - 상품 상세
	public ItemVO getItem(int item_num)
	                      throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public void updateItem(ItemVO item)throws Exception{
		...
		try {
			...
			sql = "UPDATE zitem SET name=?,price=?,quantity=?"
				+ sub_sql + ",detail=?,modify_date=SYSDATE,"
				+ "status=? WHERE item_num=?";
			pstmt = conn.prepareStatement(sql);
			//?에 데이터를 바인딩
