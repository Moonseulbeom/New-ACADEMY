...
public class CartDAO {
	...
	public void insertCart(CartVO cart)
		...
		try {
			...
			sql = "INSERT INTO zcart (cart_num,"
				+ "item_num,order_quantity,mem_num) "
	...
	public int getTotalByMem_num(int mem_num)
		...
		try {
			...
			sql = "SELECT SUM(sub_total) FROM ("
				+ "SELECT c.mem_num,"
				+ "c.order_quantity * i.price as sub_total "
				+ "FROM zcart c JOIN zitem i ON "
				+ "c.item_num=i.item_num) WHERE mem_num=?";
			pstmt = conn.prepareStatement(sql);
			//?에 데이터 바인딩
	...
	public List<CartVO> getListCart(
		...
		try {
			conn = DBUtil.getConnection();
			//SQL문 작성
			sql = "SELECT * FROM zcart c JOIN "
				+ "zitem i ON c.item_num=i.item_num "
				+ "WHERE c.mem_num = ? "
	...
	public CartVO getCart(CartVO cart)
		...
		try {
			conn = DBUtil.getConnection();
			sql = "SELECT * FROM zcart WHERE "
				+ "item_num=? AND mem_num=?";
			pstmt = conn.prepareStatement(sql);
			//?에 데이터 바인딩
			pstmt.setInt(1, cart.getItem_num());
			...
			if(rs.next()) {
				...
				cartSaved.setCart_num(
						rs.getInt("cart_num"));
				cartSaved.setItem_num(
						rs.getInt("item_num"));
				cartSaved.setOrder_quantity(
					rs.getInt("order_quantity"));
			}
	...
	//장바구니 상품번호와 회원번호별 수정
	public void updateCartByItem_num(
			                 CartVO cart)
	                       throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
