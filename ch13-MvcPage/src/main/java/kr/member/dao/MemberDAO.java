...
public class MemberDAO {
	...
	public void insertMember(MemberVO member)
		...
		try {
			...
			//zmember 테이블에 데이터 저장
			sql = "INSERT INTO zmember (mem_num,id) "
				+ "VALUES (?,?)";
			...
			sql = "INSERT INTO zmember_detail (mem_num,"
				+ "name,passwd,phone,email,zipcode,"
	...
	public MemberVO getMember(int mem_num)
		...
		try {
			...
			conn = DBUtil.getConnection();
			//SQL문 작성
			sql = "SELECT * FROM zmember m JOIN "
				+ "zmember_detail d ON "
	...
	public void updateMember(MemberVO member)
		...
		try {
			...
			conn = DBUtil.getConnection();
			//SQL문 작성
			sql = "UPDATE zmember_detail SET name=?,"
	...
	//프로필 사진 수정
	public void updateMyPhoto(String photo, 
		...
		try {
			...
			conn = DBUtil.getConnection();
			//SQL문 작성
			sql = "UPDATE zmember_detail SET photo=? "
				+ "WHERE mem_num=?";
			//PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			//?에 데이터를 바인딩
	...
	//관리자
	//전체글 개수, 검색글 개수
	public int getMemberCountByAdmin(
		...
		try {
			...
			sql = "SELECT COUNT(*) FROM zmember m "
