...
create table zboard_fav(
 ...
 mem_num number not null,
 constraint zboard_fav_pk 
                primary key (fav_num),
 constraint zboard_fav_fk1 
                foreign key (board_num)
                references zboard (board_num),
 constraint zboard_fav_fk2
                foreign key (mem_num)
                references zmember (mem_num)
);
...
create table zboard_reply(
 ...
 constraint zreply_fk foreign key (board_num)
                references zboard (board_num),
...
create table zcart(
 ...
 constraint zcart_item_fk1 
                foreign key (item_num)
                references zitem (item_num),
 constraint zcart_member_fk2
                foreign key (mem_num)
                references zmember (mem_num)
);
...
create table zorder_detail(
 ...
 constraint zorder_detail_pk 
                 primary key (detail_num),
 constraint zorder_detail_fk
...
create table zboardlevel(
 ...
 depth number not null, --자식글의 깊이. 부모글의 자식글A 1, 자식글A의 자식글B 2, 부모글의 경우 0
 ip varchar2(40) not null,
 image varchar(150),
 mem_num number not null,
 constraint zboardlevel_pk primary key (boardv_num),
 constraint zboardlevel_fk foreign key (mem_num) 
                          references zmember (mem_num)
);
